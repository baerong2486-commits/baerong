...
메인은 16:9 비율로 고정돼 있어 iframe 높이를 크게 줘도 사진이 늘어나지 않습니다.
넓은 화면(PC)에서는 내용이 630 정도로 끝나므로, 990 을 쓰면 그 아래로 360 정도가 남습니다.
남는 자리는 다른 페이지와 같은 배경 사진(연하게)이 채웁니다.
PC 여백을 완전히 없애려면 630 을 쓰면 되지만, 그러면 휴대폰에서 아래쪽이 잘립니다.
...
- **배경 사진** 🌸 사이트 탭에서 이미지 주소만 넣으면 메인·전 페이지가 한 번에 바뀜.
  ...
  휴대폰 화면은 사진이 좁게 잘리므로 **오른쪽 80% 지점**을 기준으로 보여줍니다.
  인물이 사진 왼쪽에 있는 사진으로 바꾸면 `index.html` 의 `--photo-pos-m` 값을 조정해야 합니다
